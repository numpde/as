...
\begin{frame}[t]{Single-cell RNA-seq}{}
	...
	\item<3->[C.]
		find genes with differential distributions across conditions;
...
\begin{frame}[t]{\TITLE}{``BackSPIN'' \cite{ZeiselETAL2015}}
	...
	{\only<1>{%
		...
		\item
			A matrix of weights $W$ (e.g.~concentrated on the diagonal).
		\end{itemize}
...
\begin{frame}[t]{\TITLE}{``BackSPIN'' \cite{ZeiselETAL2015}}
	...
	{\only<1>{%
		...
		\begin{itemize}
		\item
			Sort the expression matrix by both cells and genes.
		\item
