...
\begin{frame}[t]{Single-cell RNA-seq}{}
	...
	\item<1->[A.]
		...
		{\only<1>{%
			...
			\item
				Monocle
				\cite{TrapnellETAL2014}
				\hfill
				(independent component analysis and MST)
			\item
				Waterfall
				\hfill
				(spectral $k$-means + MST on centroids)
			\item
				Sincell
				\hfill
				(R workflow for building cell hierarchies)
			\item
				Oscope
				\hfill
				($k$-medoids clustering to identify oscillatory genes)
			\item
				Wanderlust
				\hfill
				(ensemble of $k$-nearest neighbor graphs, ...)
			\end{itemize}
			
			Note:
			47+ algorithms
			on single-cell pseudotime estimation \cite{Gitter2017-github}
		}}%
	\item<2->[B.]
		...
		{\only<2>{%
			...
			\item
				Destiny
				\hfill
				(extends diffusion maps to handle zeros, ...)
			\item
				SCUBA
				\hfill
				($k$-means along a temporal tree)
			\item
				BackSPIN
				\hfill
				(biclustering simultaneously clustering genes and cells)
			\item
				PAGODA
				\hfill
				(weighted PCA for gene sets)
			\end{itemize}
		}}%
	\item<3->[C.]
		...
		{\only<3>{%
			...
			\adjustbox{valign=c}{%
				\begin{minipage}{0.45\textwidth}
					\begin{overpic}[width=\textwidth]{rnaseq-de}
						%\put (0, 45) {\color{red} \tiny img: \cite{2015-Finotello-DiCamillo}}
					\end{overpic}
				\end{minipage}
			}%
			\adjustbox{valign=c}{%
				...
				\begin{minipage}{0.43\textwidth}
					\scriptsize
					``Identify genes with statistically different 
					expression levels in the compared conditions (e.g.~A and B).''
					\cite{2015-Finotello-DiCamillo}
				\end{minipage}
			}%
		...
		{\only<4>{%
			...
			\item
				MAST
				\hfill
				(logistic regression on DE between groups, ...)
			\item
				SCDE
				\hfill
				(gene-specific expression as Poisson--Binomial mixture)
			\item
				scDD
				\hfill
			...
			Reviews are e.g.~\cite{2013-Rapaport, 2015-Finotello-DiCamillo}
		}}%
	\end{itemize}
	...
	\Bottom{
		...
		\begin{references}
		{\only<1>{%
			\item[\cite{TrapnellETAL2014}]
				\bibentry{TrapnellETAL2014}
				\ 
			\item[\cite{Gitter2017-github}]
				\bibentry{Gitter2017-github}
				\ 
		}}
		{\only<4>{%
			\item[\cite{2013-Rapaport}]
				\bibentry{2013-Rapaport}
				\ 
		}}%
		{\only<3-4>{%
			\item[\cite{2015-Finotello-DiCamillo}]
				\bibentry{2015-Finotello-DiCamillo}
				\ 
		}}
		{{%
...
\def\TITLE{A. Pseudo-temporal ordering and inference}
\def\REF{
	\tiny
	\begin{references}
...
\begin{frame}[t]{\TITLE}{``Wanderlust'' \cite{BendallETAL2014}}
...
\begin{frame}[t]{\TITLE}{``Wanderlust'' \cite{BendallETAL2014}}
	...
	\adjustbox{valign=c}{%
		\begin{minipage}[t]{0.3\textwidth}
			\begin{overpic}[width=\textwidth]{wanderlust-graphxy}
		% 	\begin{overpic}[width=0.3\textwidth,grid,tics=10]{wanderlust-graphxy}
...
\begin{frame}[t]{\TITLE}{``Wanderlust'' \cite{BendallETAL2014}}
	\adjustbox{valign=t}{%
		\begin{minipage}{0.6\textwidth}
			\begin{overpic}[width=\textwidth]{wanderlust-expression}
			\end{overpic}
...
%
%
\begin{frame}[t]{\TITLE}{``Wishbone'' \cite{SettyETAL2016}}
	...
	\adjustbox{valign=t}{%
		\begin{minipage}{0.4\textwidth}
			{\only<1-3>{%
				\begin{overpic}[width=\textwidth]{wishbone-graphsjt}
				\end{overpic}
				
				\tiny
				\vspace{-5pt}
				images: \cite{SettyETAL2016}
			}}%
			{\only<4>{%
				\centering
				\begin{overpic}[width=0.7\textwidth]{wishbone-secondev}
				\end{overpic}
	...
	\adjustbox{valign=t}{%
		\begin{minipage}[t]{0.45\textwidth}
			...
			{\only<3-4>{%
				\begin{overpic}[width=0.9\textwidth]{wishbone-distancematrix}
				\end{overpic}
...
\def\TITLE{B. Identifying cell sub-populations}
%
%
\begin{frame}[t]{\TITLE}{``BackSPIN'' \cite{ZeiselETAL2015}}
	
	First, the ``SPIN'' algorithm for distance matrix reordering \cite{TsafrirTsafrirEindorZukNottermanDomany2005}.

	{\only<1>{%
		Input:
		\begin{itemize}
		\item
			A distance matrix $D \in \IR^{n \times n}$.
		\item
			A matrix of weights $W$ (e.g.~concentrated toward the diagonal).
		\end{itemize}
		
		Output: permutation $\sigma$ (near-)minimizing
		\[
			\sum_{i j} W_{i j} D_{\sigma(i) \sigma(j)}
			.
		\]
		
		``The emphasis of SPIN is on providing an informative image of
		the data, one that facilitates extraction of meaningful characteristics.''
	}}%
	{\only<2>{%
		\begin{center}
			\begin{overpic}[width=0.8\textwidth]{spin-bar}
				\put (0, 0) {\tiny img: \cite{TsafrirTsafrirEindorZukNottermanDomany2005}}
			\end{overpic}
		\end{center}
		\scriptsize
		...
		Distance matrix reordered
	}}%
	
	\Bottom{
		\tiny
		\begin{references}
		\item[\cite{TsafrirTsafrirEindorZukNottermanDomany2005}]
			\bibentry{TsafrirTsafrirEindorZukNottermanDomany2005}
			\ 
		\item[\cite{ZeiselETAL2015}]
			\bibentry{ZeiselETAL2015}
			\ 
		\end{references}
	}
\end{frame}
...
\begin{frame}[t]{\TITLE}{``BackSPIN'' \cite{ZeiselETAL2015}}
	...
	{\only<1>{%
		...
		\begin{itemize}
		\item
			Sort the cell-gene \textbf{expression} matrix by both cells and genes.
		\item
			Splitting over the cell dimension balancing sum-of-correlations
			
			--- if the sums differ significantly.
		\item
			Assign genes to each group by higher average expression.
		\item
			Repeat on submatrices.
		\end{itemize}
		
		Main point:
		``genes enriched outside of
		the cells under consideration are excluded. So, for example, Rasgrf2 would be
		excluded when clustering oligodendrocytes because it is enriched in neurons, and
		conversely, Mog would be excluded when clustering neurons.''
	}}%
	{\only<2>{%
		\vspace{-0.5cm}
		\adjustbox{valign=c}{%
			\begin{minipage}[t]{0.4\textwidth}
				\scriptsize
				``Molecular census of [mouse] somatosensory S1 cortex and hippocampus CA1 by unbiased sampling and single-cell RNA-seq''
				\cite{ZeiselETAL2015}
			\end{minipage}
		}%
		\adjustbox{valign=c}{%
			\begin{minipage}[t]{0.55\textwidth}
				\begin{overpic}[width=\textwidth]{backspin-gene-cell}
					%\put (0, 90) {\color{red} \tiny img: \cite{ZeiselETAL2015}}
				\end{overpic}
			\end{minipage}
		}%
